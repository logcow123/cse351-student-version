...
def solve_maze(maze, pos, color):
    ...
    for move in moves:
        lock = threading.Lock()
        if start_threads:
            thread = threading.Thread(target=solve_maze, args=(maze, move, get_color()))
            threads.append(thread)
            with lock:
                thread_count+= 1
            thread.start()
        else:
            solve_maze(maze, move, color)
            start_threads = True
...
def solve_find_end(maze):
    ...
    solve_maze(maze, start_pos, COLOR)
